...
export default class ExpressionParser extends LValParser {
  ...
  +parseBlock: (
    // This is the statement node for blocks like `if {}`,
    // for bare blocks this will be null
  ...
  parseExprAtom(refShorthandDefaultPos?: ?Pos): N.Expression {
    ...
    switch (this.state.type) {
      ...
      case tt._do: {
        ...
        node.body = this.parseBlock(node, false);
  ...
  parseFunctionBody(node: N.Function, allowExpression: ?boolean): void {
    const isExpression =
      ...
      (!this.hasPlugin("lenient")
        ...
        : this.hasPlugin("lenientCompat")
          ? !this.match(tt.braceL) && !this.matchIndent(node)
          : this.match(tt.braceL)
            ? this.state.input[this.state.pos] !== ";"
            : !this.matchIndent(node));
    ...
    if (isExpression) {
      ...
      this.state.inFunction = true;
      this.state.labels = [];
      node.body = this.parseBlock(node, true);
      this.state.inFunction = oldInFunc;
